...
export default function TestimonialsStaticGrid() {
  ...
  return (
    <section className="w-full px-8 py-8 md:py-16 bg-white">
      <div className="max-w-[1640px] mx-auto text-center mb-10">
      ...
      {/* Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 max-w-[1640px] mx-auto">
        {currentTestimonials.map((item, index) => (
          <div
            key={index}
            className="bg-[#373333] rounded-2xl p-6 flex flex-col cursor-pointer hover:shadow-lg transition"
      ...
      <div className="flex justify-center gap-3 mt-8">
        {Array.from({ length: totalPages }).map((_, i) => (
          <button
            key={i}
            onClick={() => setPage(i)}
            className={`w-10 h-10 flex items-center justify-center rounded-md shadow font-medium transition ${
              page === i
      ...
      {activeTestimonial && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="bg-white max-w-xl w-full rounded-2xl p-6 relative">
            ...
            {/* Content */}
            <h3 className="text-2xl font-bold mb-4">{activeTestimonial.name}</h3>
            ...
            <p className="text-gray-700 leading-relaxed">{activeTestimonial.review}</p>
          </div>
        </div>
      )}
    </section>
