"use client";

import React from "react";
import {
  FaUserSecret,
  FaShareAlt,
  FaFileExcel,
  FaExclamationTriangle,
} from "react-icons/fa";

export interface CommonMistakeItem {
  id: string;
  icon: React.ReactNode;
  title: string;
  description: string;
}

export interface FDCommonMistakesSectionProps {
  title?: string;
  subTitle?: string;
  items?: CommonMistakeItem[];
}

const defaultMistakeItems: CommonMistakeItem[] = [
  {
    id: "pre-arrest-investigation-errors",
    icon: <FaUserSecret className="w-6 h-6 sm:w-7 sm:h-7 text-primary group-hover:text-slate-950 transition-colors duration-300" />,
    title: "Pre-Arrest Investigation Errors",
    description:
      "Talking to investigators without an attorney gives the state valuable evidence they will weaponize against you later. Handing over bank statements or corporate records voluntarily bypasses key legal protections that a formal warrant ensures. Do not assume an accounting mistake proves criminal intent since prosecutors must still prove a deliberate crime.",
  },
  {
    id: "post-arraignment-missteps",
    icon: <FaShareAlt className="w-6 h-6 sm:w-7 sm:h-7 text-primary group-hover:text-slate-950 transition-colors duration-300" />,
    title: "Post-Arraignment Missteps",
    description:
      "Venting about your case on social media networks creates a permanent digital trail for prosecutors to use. Delaying representation means missing strict legal windows to secure vital evidence or dispute the state's fast-moving timeline. Contacting an accuser to settle things privately is almost always viewed by law enforcement as a form of intimidation.",
  },
  {
    id: "altering-business-records",
    icon: <FaFileExcel className="w-6 h-6 sm:w-7 sm:h-7 text-primary group-hover:text-slate-950 transition-colors duration-300" />,
    title: "Altering Business Records",
    description:
      "Deleting important emails or wiping corporate servers out of sheer panic immediately makes you look incredibly guilty. Changing regular business documents during an active investigation often triggers separate and severe criminal obstruction of justice charges. Keeping your original data intact while allowing your defense team to manage document production protects your legal strategy.",
  },
];

export default function FDCommonMistakesSection({
  title = "Common Mistakes That Risk Your Fraud Case Before You Hire a Lawyer",
  subTitle = "Plenty of fraud cases get weaker before a defense lawyer ever steps in. Avoiding these missteps early on protects both your rights and your case. A fraud defense lawyer Tampa can help you sidestep these mistakes. Call before you take any other step.",
  items = defaultMistakeItems,
}: FDCommonMistakesSectionProps) {
  return (
    <section className="w-full bg-slate-50/70 py-10 md:py-16 text-slate-800 font-sans relative overflow-hidden">
      <div className="max-w-[1640px] mx-auto px-4 sm:px-6 md:px-10">
        <div className="flex flex-col items-center justify-center text-center max-w-4xl mx-auto mb-10 md:mb-16">
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-[42px] font-bold text-slate-900 leading-tight tracking-tight mb-3">
            {title}
          </h2>
          <p className="text-slate-600 text-sm sm:text-base md:text-lg leading-relaxed max-w-3xl font-sans">
            {subTitle}
          </p>

          <div className="flex items-center justify-center gap-3 mt-5">
            <div className="w-12 sm:w-16 h-[2px] bg-primary/50 rounded-full" />
            <div className="w-8 h-8 rounded-full border border-primary/60 bg-primary/10 flex items-center justify-center text-primary shadow-sm">
              <FaExclamationTriangle className="w-4 h-4 text-primary" />
            </div>
            <div className="w-12 sm:w-16 h-[2px] bg-primary/40 rounded-full" />
          </div>
        </div>

        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-white rounded-2xl p-6 sm:p-7 border border-slate-200/80 shadow-md hover:shadow-2xl hover:-translate-y-1.5 hover:border-primary/50 transition-all duration-300 group flex flex-col items-start relative overflow-hidden cursor-pointer h-full"
              >
                <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-primary to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

                <div className="flex items-start gap-4 mb-4 w-full">
                  <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-white border border-primary/40 flex items-center justify-center shrink-0 shadow-md group-hover:scale-105 transition-all duration-300">
                    {item.icon}
                  </div>

                  <h3 className="text-base sm:text-lg font-bold text-slate-900 leading-snug pt-1 group-hover:text-primary transition-colors duration-300">
                    {item.title}
                  </h3>
                </div>

                <div className="w-12 group-hover:w-24 h-[2px] bg-primary/70 rounded-full mb-4 ml-1 transition-all duration-300" />

                <p className="text-xs sm:text-sm text-slate-600 leading-relaxed font-sans flex-grow">
                  {item.description}
                </p>
              </div>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}
